...
public class SampleManagerConnector {
	...
	public static enum SQL {
		...
		CREATE_INDEX_LOCKED_TIMESTAMP_LOCATION("CREATE INDEX IF NOT EXISTS IDX_LOCKED_TIMESTAMP_LOCATION ON SAMPLE (LOCKED, TIMESTAMP, LOCATION)"),
		...
		QUERY_ALL("SELECT ID, PROJECT, META_PATH, SAMPLE_PATH, LOCATION, TIMESTAMP, LAST_MODIFIED, LOCKED, DEVICE FROM SAMPLE WHERE NOT LOCKED ORDER BY TIMESTAMP, LOCATION"),
		...
		QUERY_ALL_PAGED("SELECT ID, PROJECT, META_PATH, SAMPLE_PATH, LOCATION, TIMESTAMP, LAST_MODIFIED, LOCKED, DEVICE FROM SAMPLE USE INDEX (IDX_LOCKED_TIMESTAMP_LOCATION) WHERE NOT LOCKED ORDER BY TIMESTAMP, LOCATION LIMIT ? OFFSET ?"),
	...
	public void init(boolean clear) {
		try {
			...
			getStatement(SQL.CREATE_INDEX_LOCKED_TIMESTAMP_LOCATION).executeUpdate();			
		} catch (SQLException e) {
			throw new RuntimeException(e);
		}
	}
